...
async def save_uploaded_file(file: UploadFile) -> tuple[str, str]:
    ...
    if settings.storage_backend == "s3":
        ...
        file_path = safe_path_join(DOCUMENT_UPLOAD_DIR, stored_filename)
        content = await file.read()
...
async def get_file_path(filename: str) -> Path:
    ...
    """
    local_path = safe_path_join(DOCUMENT_UPLOAD_DIR, filename)
...
async def delete_file(filename: str):
    ...
    if settings.storage_backend == "s3":
        ...
        try:
            s3.delete_object(
                Bucket=settings.s3_project_documents_bucket, Key=f"documents/{filename}"
            )
        except Exception as e:
...
def validate_avatar_file(filename: str, file_size: int) -> tuple[bool, str]:
    ...
    if file_size > MAX_AVATAR_FILE_SIZE:
        return (
            False,
            f"File size exceeds maximum of {MAX_AVATAR_FILE_SIZE // (1024 * 1024)}MB",
        )
...
async def save_avatar_file(file: UploadFile, avatar_uuid: str) -> tuple[str, str, int]:
    ...
    if settings.storage_backend == "s3":
        ...
        local_path = safe_path_join(AVATAR_UPLOAD_DIR, stored_filename)
        # bearer:disable python_lang_path_traversal
...
def get_avatar_path(filename: str) -> Path:
    ...
    return safe_path_join(AVATAR_UPLOAD_DIR, filename)
...
async def delete_avatar_file(file_path: str):
    ...
    if settings.storage_backend == "s3":
        s3 = get_s3_client()
        try:
            s3.delete_object(Bucket=settings.s3_avatars_bucket, Key=file_path)
        except Exception as e:
            print(f"Error deleting avatar from S3: {e}")
    else:
        local_path = safe_path_join(AVATAR_UPLOAD_DIR, file_path)
        if local_path.exists():
            local_path.unlink()
...
async def save_logo_file(file: UploadFile, project_uuid: str) -> str:
    ...
    if settings.storage_backend == "s3":
        ...
        file_path = safe_path_join(LOGO_UPLOAD_DIR, stored_filename)
...
def get_logo_path(filename: str) -> Path:
    ...
    return safe_path_join(LOGO_UPLOAD_DIR, filename)
