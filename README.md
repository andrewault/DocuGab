...
The chat interface supports four distinct types of response bubbles, each serving a specific purpose:

### 1. LLM Response
**Appearance:** White bubble with black text.
This is the standard text response generated by the local LLM (Ollama). It synthesizes information from your documents to answer questions. It supports Markdown formatting including tables, code blocks, and lists.

### 2. Sources Bubble
**Appearance:** Green bubble (`#dcfce7`) with dark green text.
When the AI uses your documents to answer a question, it cites its sources here.
- Lists the specific files and chunks used for context.
- Clicking a source citation opens the specific document page in a new tab.

### 3. Ancillary Responses
**Appearance:** Dark Blue bubble (`#1e1b4b`) with white text.
Triggered by specific keywords in the chatbot's response, this bubble provides rich media and external resources managed in the Admin panel.
- **Media**: Embeds YouTube videos and displays photos directly in the chat.
- **Links**: Provides clickable lists of relevant external URLs (e.g., official websites, maps).
- *Example*: Asking about "Balboa Park" might trigger a photo of the park and a link to its official site.

### 4. Built-in Responses
**Appearance:** Standard LLM bubble (White).
For common social conversational inputs like "Hello", "Hi", or "How are you?", the system bypasses the expensive RAG pipeline to provide an instant, natural HTTP response. These responses do not search your documents.

![DocuTok Interface](docs/screenshot.png)
